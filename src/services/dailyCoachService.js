...
export async function generateDailyMessage() {
  ...
  const hour       = new Date().getHours()
  const timeOfDay  = hour < 12 ? 'בוקר טוב' : hour < 17 ? 'אחר הצהריים' : 'ערב טוב'
  const streakLine = streak > 0 ? `${streak} ימים ברצף.` : 'אפס ימים ברצף.'
  const trackDesc  = activeTrackName ? `מסלול ${activeTrackName}` : 'המשימה שלך'

  const prompt =
    `כתוב בדיוק 2 משפטים בעברית תקנית. אל תוסיף כלום מעבר לכך.\n\n` +
    `משפט 1: פתח ב"${timeOfDay}." — לאחר מכן ציין: ${streakLine} לאחר מכן משפט ישיר ותובעני על ${trackDesc}.\n` +
    `משפט 2: קריאה לפעולה ספציפית והחלטית לסגירת היום.\n\n` +
    `הקשר: דירוג ${rankLabel}, ציון ${score}` +
    (missedDays >= 3 ? `, ${missedDays} ימים שפוספסו — תעמת עם זה ישירות.` : '.') + `\n\n` +
    `חוקים מחייבים:\n` +
    `- עברית תקנית בלבד. כל מילה חייבת להיות מילה שלמה וקיימת במילון.\n` +
    `- אסור: markdown, כוכביות, מירכאות, קיצורים, ניקוד, סימני פיסוק מיוחדים.\n` +
    `- אסור: המצאת מילים, חיבור מילים, קיצור מילים. רק מילים עבריות מלאות.\n` +
